...
public class ThreadedEchoServer extends EchoServer implements Runnable {
	...
	/**
	 * The new version of handleConnection starts a thread. This new thread will
	 * call back to the <I>old</I> version of handleConnection, resulting in the
	 * same server behavior in a multithreaded version. The thread stores the
	 * Socket instance since run doesn't take any arguments, and since storing
	 * the socket in an instance variable risks having it overwritten if the
	 * next thread starts before the run method gets a chance to copy the socket
	 * reference.
	 */
...
/**
 * This is just a Thread with a field to store a Socket object. Used as a
 * thread-safe means to pass the Socket from handleConnection to run.
 */
