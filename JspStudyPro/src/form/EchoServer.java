 ...
 * received from the Web client (usually a browser). To use this server, start
 * it on the system of your choice, supplying a port number if you want
 * something other than port 8088. Call this system server.com. Next, start a
 * Web browser on the same or a different system, and connect to
 * http://server.com:8088/whatever. The resultant Web page will show the data
 * that your browser sent. For debugging in a servlet or other server-side
 * program, specify http://server.com:8088/whatever as the ACTION of your HTML
 * form. You can send GET or POST data; either way, the resultant page will show
...
public class EchoServer extends NetworkServer {
	...
	/**
	 * Supply a port number as a command-line argument. Otherwise, use port
	 * 8088.
	 */
	...
	/**
	 * Overrides the NetworkServer handleConnection method to read each line of
	 * data received, save it into an array of strings, then send it back
	 * embedded inside a PRE element in an HTML page.
	 */

	public void handleConnection(Socket server) throws IOException {
		System.out.println(serverName + ": got connection from " + server.getInetAddress().getHostName());
